...
import type { TooltipTriggerProps, TooltipTriggerType } from './types';
...
export const TooltipTrigger = ({
  ...
  const childElement = children as ReactElement<TooltipChildProps>;
  const overlayElement = overlay as ReactElement<TooltipOverlayProps> | undefined;
  ...
  if (!isValidElement(children)) {
    throw new Error('TooltipTrigger 的 children 必须是单个 React 元素');
  }

  const hasOverlay = overlayElement != null;
  const hasTitle = !hasOverlay && title != null && title !== '';
  const canShow = hasOverlay || hasTitle;
  const show = canShow && (showProp ?? showState);
  const visible = show && !disabled;

  const tooltipId = id ?? overlayElement?.props.id ?? generatedId;
  ...
  useEffect(() => {
    if (!transition.mounted || !tooltipElement) {
      return;
    }
    const handleTransitionEnd = (event: TransitionEvent) => {
      if (event.target !== tooltipElement || event.propertyName !== 'opacity') {
        return;
      }
      dispatch({ type: 'ANIMATION_END' });
    };
    tooltipElement.addEventListener('transitionend', handleTransitionEnd);
    return () => tooltipElement.removeEventListener('transitionend', handleTransitionEnd);
  }, [tooltipElement, transition.mounted]);
  ...
  useLayoutEffect(() => {
    ...
    if (!transition.mounted || !tooltipElement || !triggerElement) {
      return;
    }

    const applyPosition = () => {
      const nextPlacement = positionElement(tooltipElement, triggerElement, {
        flip,
        offset,
        padding,
        placement,
    ...
    return () => {
      ...
      resetPosition(tooltipElement);
    };
  }, [flip, offset, padding, placement, tooltipElement, transition.mounted]);

  const triggerList = useMemo<TooltipTriggerType[]>(
  ...
  const setTooltipElementCallback = useCallback((element: HTMLElement | null) => {
  ...
  let tooltipNode: null | ReactElement = null;

  if (transition.mounted && overlayElement) {
    // oxlint-disable-next-line react/refs
    tooltipNode = cloneElement(overlayElement, {
